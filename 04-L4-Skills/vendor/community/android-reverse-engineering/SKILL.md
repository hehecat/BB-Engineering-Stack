...
## BB Engineering Stack Integration

This vendored snapshot runs inside `bb-stack`. Use paths rooted at
`$BB_STACK_ROOT`, keep generated output inside the active Engagement, and use
`bb-stack bootstrap --profile android` for required dependencies. Do not run
the bundled dependency installer during a managed session. Vineflower and
dex2jar remain optional and are not required for the default JADX workflow.
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/check-deps.sh
```

On Windows (PowerShell):
```powershell
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/check-deps.ps1"
```

If anything is missing, follow the installation instructions in `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/setup-guide.md`.
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/fingerprint.sh <file.apk|file.xapk>
```
...
**If required dependencies are missing** (exit code 1), return to the stack
root and repair the managed Android profile:
...
Do not install optional dependencies unless the selected analysis actually needs
them. The default Headless workflow uses the stack-pinned JADX.
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/decompile.sh [OPTIONS] <file>
```

On Windows (PowerShell):
```powershell
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/decompile.ps1" [OPTIONS] <file>
```
...
See `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/jadx-usage.md` and `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/fernflower-usage.md` for the full CLI references.
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/recover-kotlin-names.sh \
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/lookup-name.sh \
...
See `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/kotlin-name-recovery.md`
...
See `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/call-flow-analysis.md` for detailed techniques and grep commands.
...
```bash
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.sh <output>/sources/
```

On Windows (PowerShell):
```powershell
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.ps1" <output>/sources/
```

Targeted searches:
```bash
# Only Retrofit
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.sh <output>/sources/ --retrofit

# Only hardcoded URLs
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.sh <output>/sources/ --urls

# Only auth patterns
bash ${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.sh <output>/sources/ --auth
```

On Windows (PowerShell):
```powershell
# Only Retrofit
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.ps1" <output>/sources/ -Retrofit

# Only hardcoded URLs
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.ps1" <output>/sources/ -Urls

# Only auth patterns
& "${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/scripts/find-api-calls.ps1" <output>/sources/ -Auth
```
...
See `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/api-extraction-patterns.md` for library-specific search patterns and the full documentation template.
...
- `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/setup-guide.md` — Installing Java, jadx, Fernflower/Vineflower, dex2jar, and optional tools
- `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/jadx-usage.md` — jadx CLI options and workflows
- `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/fernflower-usage.md` — Fernflower/Vineflower CLI options, when to use, APK workflow
- `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/api-extraction-patterns.md` — Library-specific search patterns and documentation template
- `${BB_STACK_ROOT}/04-L4-Skills/vendor/community/android-reverse-engineering/references/call-flow-analysis.md` — Techniques for tracing call flows in decompiled code
